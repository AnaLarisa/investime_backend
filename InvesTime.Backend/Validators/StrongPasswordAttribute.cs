...
{
    private new const string ErrorMessage =
    ...
    {
        if (value == null) return new ValidationResult("The password provided is null.");

        if (value is string password && !IsStrongPassword(password)) return new ValidationResult(ErrorMessage);
